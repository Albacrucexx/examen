...
async function testApi() {
  try {
    ...
    let res = await axios.get(`http://localhost:${port}/ld`);
    ...
    res = await axios.post(`http://localhost:${port}/ld`, nuevo);
    console.log(res.data);

    console.log("=== GET después del POST ===");
    res = await axios.get(`http://localhost:${port}/ld`);
    console.log(res.data);

    console.log("=== DELETE del nuevo equipo ===");
    await axios.delete(`http://localhost:${port}/ld/${res.data.at(-1).id}`);
